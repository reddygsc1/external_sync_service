...
class BaseEvent(BaseModel):
    """Base event schema for all record types"""
...
class InternalContact(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=255, description="Full name of the contact"
    )
    email: EmailStr = Field(..., description="Email address")
    phone: str = Field(..., min_length=10, max_length=20, description="Phone number")
    contact: Literal[
        "lead", "customer", "prospect", "vendor", "partner", "employee"
...
class InternalContactEvent(BaseEvent):
    """Internal contact event schema"""

    record: Literal["contacts"] = Field(..., description="Record type must be contacts")
