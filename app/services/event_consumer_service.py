...
from .schema_transformer import SchemaTransformer, SchemaTransformerFactory, ExternalSystem
...
class InternalEventConsumer(EventConsumerService):
    ...
    def __init__(self, transformer_config: Optional[Dict[str, Any]] = None):
        ...
        self.schema_transformer = SchemaTransformer(transformer_config)
    ...
    async def consume_event(self, event: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Transformed contact {item.get('id')} to {external_system} format")
                
            except Exception as e:
                logger.error(f"Schema transformation error for contact {item.get('id')}: {e}")
            ...
            logger.info(f"Processed {operation} event for contact {item.get('id')} ({contact_type})")
            return True
    ...
    async def consume_batch(self, events: List[Dict[str, Any]]) -> bool:
        ...
        try:
            results = []
            for event in events:
                result = await self.consume_event(event)
                results.append(result)
            
            success_count = sum(results)
            total_count = len(events)
            
            logger.info(f"Batch processed: {success_count}/{total_count} events successful")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_processed": self.processed_events[-1]["received_at"] if self.processed_events else None
        }
    ...
    def get_transformation_summary(self) -> Dict[str, Any]:
        ...
        successful_transformations = sum(1 for event in self.processed_events if event.get("transformation_success"))
        ...
        return {
            ...
            "success_rate": (successful_transformations / total_events * 100) if total_events > 0 else 0,
...
class MockEventConsumer(EventConsumerService):
    ...
    async def consume_event(self, event: Dict[str, Any]) -> bool:
        ...
        logger.info(f"Mock consumer received: {event['operation']} event for {event['item']['id']}")
        return True
