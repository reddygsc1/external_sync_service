...
class BaseAdapter(ABC):
    ...
    def add_metadata(
        self,
        data: Dict[str, Any],
        internal_data: Union[InternalContact, Dict[str, Any]],
    ) -> Dict[str, Any]:
        """Add metadata to transformed data"""
        contact_id = (
            internal_data.id
            if isinstance(internal_data, InternalContact)
            else internal_data.get("id", "")
        )
        contact_type = (
            internal_data.contact
            if isinstance(internal_data, InternalContact)
            else internal_data.get("contact", "")
        )
